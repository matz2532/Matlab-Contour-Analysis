...
function [all_cell_labels, all_anisotropy_values, all_orientation_values] = analyse_intensity_in_LEC(watershedded_labelled_image, intensity_projection_image, cell_outlines, plot_contour_on_labelled_image)
...
for i = 1:length(cell_outlines);    
    ...
    if plot_contour_on_labelled_image == true
        imagesc(watershedded_labelled_image)
        hold on
        plot(contour_0{1}(:,2), contour_0{1}(:,1))
        hold on
        filename = append('labelledImageWithContour_', string(cell), '.tiff');
        saveas(gcf, filename);    
        close figure 1
    end

    % fit circle to the cell outline 
    [xc, yc, R] = circfit(contour_0{1}(:,2), contour_0{1}(:,1));
    
    % make circle range dependent on R
    th = 0:pi/50:2*pi;

    % create circle outline
    segment_x = R*cos(th)+xc; 
    segment_y = R*sin(th)+yc;
